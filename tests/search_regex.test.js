...
describe('--search-regex', () => {
  test('should return tables matching a regular expression', () => {
    const { status, json } = runCli([TABLES_DIR, '--search-regex', 'customer_.*address']);
  ...
  test('should match any of multiple regex patterns', () => {
    const { status, json } = runCli([TABLES_DIR, '--search-regex', '^customers$', 'address']);
  ...
  test('should return an empty array when nothing matches', () => {
    const { status, json } = runCli([TABLES_DIR, '--search-regex', '^zzz_no_match$']);
  ...
  test('should fail when a regex pattern is invalid', () => {
    const { status, error } = runCli([TABLES_DIR, '--search-regex', '[invalid']);
  ...
  test('should fail when used together with --search', () => {
    const { status, error } = runCli([TABLES_DIR, '--search', 'customer', '--search-regex', '.*']);
