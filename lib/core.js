...
function compileSearchRegex(pattern) {
  try {
    ...
    throw new ScoutError(`Invalid search regex pattern '${pattern}': ${error.message}`);
  }
}

export function searchTablesRegex(folder, patterns) {
  const targetFolder = assertFolder(folder);
  const regexes = patterns.map((pattern) => compileSearchRegex(pattern));
  return listTableFiles(targetFolder).filter((name) => regexes.some((regex) => regex.test(name)));
}
...
export function getKeysInfo(folder, table) {
  const targetFolder = assertFolder(folder);
  const ast = loadAndParseDDL(targetFolder, table);
  return buildKeysInfo(ast);
}

export function getKeysInfoMany(folder, tables) {
  const targetFolder = assertFolder(folder);
  let hasError = false;
  ...
  const data = tables.map((table) => {
    ...
    return { name: table, ...buildKeysInfo(node) };
  });

  return { data, exitCode: hasError ? 1 : 0 };
}

export function resolveKeysInfo(folder, tables) {
  if (tables.length === 1) {
    return { data: getKeysInfo(folder, tables[0]), exitCode: 0 };
  }

  return getKeysInfoMany(folder, tables);
}

function findReferencedBy(folder, table) {
  const targetFolder = assertFolder(folder);
  loadAndParseDDL(targetFolder, table);
...
export function getReferences(folder, table) {
  const targetFolder = assertFolder(folder);
  const ast = loadAndParseDDL(targetFolder, table);
  const references = extractForeignKeys(ast.create_definitions || []);

  return { name: table, references };
}

export function getReferencedBy(folder, table) {
  return { name: table, referenced_by: findReferencedBy(folder, table) };
}
...
export function getRelations(folder, table) {
  ...
  const ast = loadAndParseDDL(targetFolder, table);
  const references = extractForeignKeys(ast.create_definitions || []);
  const referencedBy = findReferencedBy(targetFolder, table);
