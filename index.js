...
program
  ...
  .option('--references <table>', 'Return outgoing foreign keys declared by a table')
  .option(
    '--referenced_by <table>',
...
function runCommand(targetFolder, options) {
  ...
  if (options.references) {
    succeed(core.getReferences(targetFolder, options.references));
    return;
  }

  if (options.referenced_by) {
    succeed(core.getReferencedBy(targetFolder, options.referenced_by));
    return;
  }
